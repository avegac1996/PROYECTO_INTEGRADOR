...
const anadirUsuario = async (nombre, apellido, correo, contrasenia) => {
  try {
    let uniqid = Math.floor(Math.random() * 10000);
    console.log(uniqid);
    const sqlStatement = `INSERT INTO administrador VALUES (${uniqid}, '${nombre}','${apellido}','${correo}','${contrasenia}')`;
    const res = await pool.query(sqlStatement);
...
app.get("/AnadirUsuarios", (req, response) => {
  let { nombre, apellido, email, password } = req.query;
  anadirUsuario(nombre, apellido, email, password)
    .then((data) => {
      console.log("GET /Anadir correcta");
      response.json(true);
    })
    .catch((err) => {
      console.error(err);
